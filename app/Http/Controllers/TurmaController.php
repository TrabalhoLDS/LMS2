...
class TurmaController extends Controller
{
    ...
    //receber o id de atividade vai request
    // o id de turma será por "seção"
    public function VincularTurmaAtividade(Request $request)
    {
        $request->validate([
        ...
        try {
            ...
            // Verifica se o usuário já está vinculado a esta turma
            if ($atividade->turmas()->where('turma_id', $turma->id)->exists()) {
                return back()->with('warning', 'Este atividade já está vinculado a esta turma!');
            }

            $turma->atividades()->associate($atividade);

            $atividade->save();
        } catch (\Exception $e) {

            return back()->with('error', 'Erro ao atividade à turma: ' . $e->getMessage());
        }
    }
}
